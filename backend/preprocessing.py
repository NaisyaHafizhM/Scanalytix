...
def fill_missing_by_method(df: pd.DataFrame, method: str = "Mean"):
    """Fill missing values across the WHOLE dataset using a single chosen
    method — user only picks Mean / Median / Modus, no column selection
    needed. Mean & Median apply to numeric columns; Modus applies to all
    columns (numeric & categorical) since it works for any data type."""
    before = snapshot(df)
    df_clean = df.copy()
    method_norm = (method or "mean").strip().lower()

    if method_norm in ("mean", "median"):
        num_cols = df_clean.select_dtypes(include="number").columns
        if len(num_cols) == 0:
            after = snapshot(df_clean)
            return df_clean, before, after, f"Tidak ada kolom numerik untuk diisi dengan {method_norm}."
        if method_norm == "mean":
            df_clean[num_cols] = df_clean[num_cols].fillna(df_clean[num_cols].mean())
        else:
            df_clean[num_cols] = df_clean[num_cols].fillna(df_clean[num_cols].median())
    ...
    after = snapshot(df_clean)
    filled = before["missing_total"] - after["missing_total"]
    return df_clean, before, after, f"Mengisi {filled} missing value di seluruh dataset menggunakan {label}."
